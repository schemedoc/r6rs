             ...
             (if probe
                 (integer->char
                  (+ n (vector-ref simple-case-adjustments
                                   (bytes-u8-ref simple-upcase-adjustments
                                                 probe))))
                 c)))
          ((<= #x10428 n #x1044f)
           (integer->char (- n 40)))
          ...
          ((<= n #xffff)
           (let ((probe (binary-search-16bit n simple-downcase-chars)))
             (if probe
                 (integer->char
                  (- n (vector-ref simple-case-adjustments
                                   (bytes-u8-ref simple-downcase-adjustments
                                                 probe))))
                 c)))
          ((<= #x10400 n #x10427)
           (integer->char (+ n 40)))
  ...
  (let ((n (char->integer c)))
    (vector-ref
     ...
     (if (< n (bytes-length
         ...
         (bytes-u8-ref general-category-indices-for-common-characters n)
         (bytes-u8-ref general-category-indices-for-all-characters
...
(do ((i 0 (+ i 1))
     (bv (make-bytes number-of-common-characters)))
...
(define simple-upcase-chars (make-bytes 0))
...
(define simple-upcase-adjustments (make-bytes 0))
...
(define simple-downcase-chars (make-bytes 0))
...
(define simple-downcase-adjustments (make-bytes 0))
...
(let ((simple-upcase-mappings
  ...
  (let ((adjustments '()))
    ...
    (set! simple-upcase-adjustments
          (make-bytes (length simple-upcase-mappings)))

    (set! simple-downcase-adjustments
          (make-bytes (length simple-downcase-mappings)))

    (set! simple-upcase-chars
          (make-bytes
           (* 2 (bytes-length simple-upcase-adjustments))))

    (set! simple-downcase-chars
          (make-bytes
           (* 2 (bytes-length simple-downcase-adjustments))))
    ...
    (do ((i 0 (+ i 1))
      ...
      (let* ((mapping (car mappings))
             (cp0 (car mapping))
             (cp1 (cadr mapping)))
        (bytes-u8-set! simple-upcase-chars j (hi-bits cp0))
        (bytes-u8-set! simple-upcase-chars (+ j 1) (lo-bits cp0))
        (bytes-u8-set! simple-upcase-adjustments
                       i
    ...
    (do ((i 0 (+ i 1))
      ...
      (let* ((mapping (car mappings))
             (cp0 (car mapping))
             (cp1 (cadr mapping)))
        (bytes-u8-set! simple-downcase-chars j (hi-bits cp0))
        (bytes-u8-set! simple-downcase-chars (+ j 1) (lo-bits cp0))
        (bytes-u8-set! simple-downcase-adjustments
                       i
...
(define special-case-chars (make-bytes 0))
...
(let ((special-case-mappings
    ...
    (let ((n (length special-case-mappings)))
      (set! special-case-chars (make-bytes (* 2 n)))
    ...
    (do ((i 0 (+ i 1))
      ...
      (let* ((mapping (car mappings))
        ...
        (bytes-u8-set! special-case-chars j (hi-bits cp0))
        (bytes-u8-set! special-case-chars (+ j 1) (lo-bits cp0))
