...
eol-style symbol acceptable as an \var{eol-mode} argument to {\cf
...
\item {\cf (\var{read!} \var{string} \var{start} \var{count})}
  ...
  The \var{read!} procedure must return an exact integer object.  This
...
do not raise an exception) are available, or until an end of
file is reached.
...
\domain{\var{Start} and \var{count} must be exact, non-negative
  integer object, with \var{count} representing the number of characters to be read.
