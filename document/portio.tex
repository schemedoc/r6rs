...
As a side effect, however, the \var{binary-port} is closed in
...
\begin{entry}{%
\pproto{(make-binary-input-port \var{id} \var{read!}}{procedure}}
\mainschindex{make-binary-input-port}{\tt\obeyspaces\\
...
The remaining arguments are procedures that will be called as
necessary by operations that perform binary input, positioning,
or closing.
The \var{get-position}, \var{set-position!}, and \var{close}
arguments may be \schfalse{} instead of a procedure, which
means (respectively) that the binary port will not support
the {\cf port-position} operation, will not support the
{\cf set-port-position!} operation, or will not require
any actions to be performed when the port is closed.
...
\item {\cf (\var{read!} \var{bytes} \var{start} \var{count})}
  ...
  and \var{bytes} will be a bytes object whose length is at least
  $\var{start} + \var{count}$.
  The \var{read!} procedure obtains up to \var{count} bytes
  from the byte source and writes those bytes
  into \var{bytes} starting at index \var{start}.
...
\rproto{open-file-output-port}{ filename file-options buffer-mode}{procedure}
\rproto{open-file-output-port}{ filename file-options buffer-mode transcoder}{procedure}}
...
\begin{entry}{%
\pproto{(make-binary-output-port \var{id} \var{write!}}{procedure}}
\mainschindex{make-binary-output-port}{\tt\obeyspaces\\
    \var{get-position} \var{set-position!} \var{close})}
...
The remaining arguments are procedures that will be called as
necessary by operations that perform binary output, positioning,
or closing.
The \var{get-position}, \var{set-position!}, and \var{close}
arguments may be \schfalse{} instead of a procedure, which
means (respectively) that the binary port will not support
...
\item {\cf (\var{write!} \var{bytes} \var{start} \var{count})}
  ...
  from \var{bytes} starting at index \var{start}, and forward
...
\item {\cf (\var{get-position})}
  ...
  The \var{get-position} procedure (if supplied)
  returns the current position of the output port.
  
\item {\cf (\var{set-position!} \var{k})}
       
  The \var{set-position!} procedure (if supplied) sets the position
  of the output port to \var{k}.
       
\item {\cf (\var{close})}
       
  The \var{close} procedure (if supplied) performs any
  actions that are necessary when the output port is closed.
\end{itemize}
