...
\var{buffer-name} for arguments that shall be buffer-mode symbols.

\begin{entry}{%
\proto{buffer-mode}{ \hyper{name}}{\exprtype}}
   
\hyper{Name} shall be one of the \meta{identifier}s {\cf none}, {\cf line}, or
{\cf block}. The result is the corresponding symbol, denoting the
associated buffer mode.

If \hyper{name} is not one of the valid identifiers, an exception shall be
...
If \var{name} is one of the \meta{identifier}s {\cf lf}, {\cf cr}, {\cf crlf}, or
{\cf ls}, then the form evaluates to the corresponding symbol.  If \var{name} is
not one of these identifiers, the effect of evaluating this expression is
...
the port's current position, and the exception handler shall update the 
...
If \var{name} is one of the \meta{identifier}s {\cf ignore}, {\cf raise}, or {\cf
  replace}, then the result is the corresponding symbol.  If \var{name} is
not one of these identifiers, the result the expression is
implementation-dependent.
...
\domain{\var{Codec} shall be a codec, \var{eol-style}, if present, an
...
\domain{\var{Proc} shall be a procedure that accepts a single
  argument.}  The {\cf call-with-prot} procedure
...
After {\cf open-bytes-input-port}
has been called, if \var{bytes} is modified, the effect on the returned
port is unspecified.
\end{entry}
...
of the input, {\cf get-bytes-n} returns a bytes object of size \var{k}. If fewer
...
\domain{\var{Count} shall be an exact, non-negative integer, specifying
  the number of bytes to be read. \var{Bytes} shall be a bytes object with at
...
the the result is \var{count}. If fewer bytes are available before
...
may block indefinitely, waiting to see an apparent end of
...
\domain{\var{Start} and \var{count} shall be an exact, non-negative
  integer, specifying the number of characters to be read.
  \var{String} shall be a string with at least $\var{start} +
...
Reads from \var{input-port} until an apparent end of the input, decoding
...
\domain{\var{output-port} shall be a port that accumulates a bytes
...
\domain{\var{Proc} shall be a procedure accepting one argument.}
...
as a freshly allocated string.  The decoding is according to
...
a freshly allocated string.  The decoding is according to
\var{transcoder} if it is specified, and UTF-8 if not.
(FIXME: Shouldn't the port's transcoder be considered?)
In either
case, decoding errors are always handled analogously to the {\cf
...
\domain{\var{Start} and \var{count} shall be non-negative exact
  ...
  - \var{start}$, respectively. \var{Bytes} shall have a size of at
...
\domain{\var{Start} and \var{count} shall be non-negative exact
  integers.  \var{String} shall have a length of at least $\var{start} +
