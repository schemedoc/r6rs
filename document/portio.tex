...
The magic procedure of \var{binary-port} becomes the magic
...
\begin{entry}{%
\proto{port-magic}{ port}{procedure}}

Returns the magic procedure associated with \var{port}.
...
If \var{bytevector} is modified after {\cf open-bytevector-input-port}
...
\mainschindex{make-custom-binary-input-port}{\tt\obeyspaces\\
    \var{get-position} \var{set-position!} \var{close} \var{magic})}
...
\item {\cf (\var{magic} \dotsfoo{})}

  The \var{magic} procedure (if supplied) will be returned
  when the custom port is passed to {\cf port-magic}.
  The \var{magic} procedure should accept any number of arguments
  of any type.  For almost all arguments, \var{magic} should
  ignore its arguments and return the unspecified value
  without performing any side effects, but \var{magic}
  may respond to some arguments by performing arbitrary
  computations that implement custom behavior.
  If \var{magic} is not supplied, then the custom port's
...
\mainschindex{make-custom-binary-output-port}{\tt\obeyspaces\\
    \var{write!} \var{get-position} \var{set-position!} \var{close} \var{magic})}
...
\rproto{put-bytevector}{ binary-output-port bytevector start count}{procedure}}
...
\mainschindex{make-custom-binary-input/output-port}{\tt\obeyspaces\\
    \var{id} \var{read!} \var{write!} \var{get-position} \var{set-position!} \var{close} \var{magic})}
