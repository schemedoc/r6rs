...
object of its own type, as are the unspecified value, and the end of
file oject.
...
\index{end of file value}\schindex{unspecified?}\schindex{eof-object?}\schindex{null?}
...
A \hyper{definition} shall have one of the following forms:\mainschindex{define}\mainindex{definition}
...
{\cf \hyper{declaration} \ldots \hyper{definition} \ldots \hyper{sequence}}
...
As a subform of a body, a {\cf begin} form acts as a splicing form and
is treated as if it were not actually present; see section~\ref{begin}.
...
\syntax
\hyper{Test}, \hyper{consequent}, and \hyper{alternate} shall be 
expressions.
...
\syntax
\hyper{Key} shall be any expression.  Each \hyper{clause} has one of
the following forms:
\begin{scheme}
...
\syntax The \hyper{test}s shall be expressions.
\semantics
...
\syntax
\hyper{Mv-bindings} shall have the form
\begin{scheme}
...
region.\index{region}
If no \hyper{formals} match, an exception with condition type
{\cf\&contract} is raised.
...
\item It may appear as a form in a \hyper{body} (see
  ...
  section~\ref{librarybodysection}), or \hyper{script body} (see
  chapter~\ref{scriptchapter}), or directly nested in a {\cf begin}
  ...
  the \hyper{body} are spliced intop the surrounding body, as if the
  {\cf begin} wrapper were not actually present.
  
  A {\cf begin} form in a \hyper{body} or \hyper{library body} shall
  be non-empty if it appears after the first \hyper{expression}
  within the body.
\item It may appear as a regular expression and shall have the shape
...
as nodes with outgoing edges, uses {\cf
  ...
  bytes=?} to compare bytes objects (see chapter~\ref{byteschapter}),
...
(/ -1 0.0)                             \ev  -inf.0
(/ +inf.0)                             \ev  0.0
(/ 0 0)                                \lev \exception{\&contract} \textit{or} +nan.0
...
shall be neither infinite nor a NaN, and \varii{x} shall be nonzero;
otherwise, an exception with condition type {\cf\&contract} is raised.
...
\var{precision} shall be an exact positive integer, and \var{radix}
...
\nodomain{All \var{list}s shall be lists.}
...
\begin{entry}{%
\proto{list-tail}{ l k}{procedure}}

\domain{If \var{k} is 0, \var{l} shall be the empty list or a pair.
  Otherwise, \var{l} shall be a chain of pairs of size at least \var{k}.}

The {\cf list-tail} procedure returns the subchain of pairs of \var{l}
...
\begin{entry}{%
\proto{list-ref}{ l k}{procedure}}

\domain{\var{L} shall be a chain of pairs of size at least $\var{k}+1$.}

Returns the \var{k}th element of \var{l}.
...
\domain{The \var{list}s shall all have the same length.  \var{proc}
  shall be a procedure.  If the \var{list}s are non-empty,
  \var{proc} shall
  take as many arguments as there are {\it list}s and shall return a
  single value.}
...
\domain{\var{K} shall be a valid index of \var{string}.}
...
\domain{\var{K} shall be a valid index of \var{vector}.}
...
\mainschindex{do}{\tt\obeyspaces%
  ...
  \hyper{expressionx} \dotsfoo)}

\syntax
The \hyper{init}s, \hyper{step}s, and \hyper{test}s shall be
expressions.  The \hyper{variable}s shall be pairwise distinct variables.
...
{\cf unquote-splicing} and multi-operand {\cf unquote} forms shall
...
\semantics
The \hyper{form}s or \hyper{definitions}s are expanded in the syntactic environment
...
Thus, internal definition in the result of expanding the \hyper{definition}s have
