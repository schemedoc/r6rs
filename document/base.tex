...
This binds the keyword \hyper{variable} to the transformer specified by
...
The {\cf equal?} predicate treats pairs, vectors and byte objects (see
section~\ref{bytessection}) as nodes with outgoing edges, uses {\cf
  string=?} to compare strings, and uses {\cf eqv?} to compare other
nodes.
...
(equal? '\#vu8(1 2 3 4 5)
        (u8-list->bytes '(1 2 3 4 5)) \lev \schtrue
(equal? (lambda (x) x)
        (lambda (y) y))  \ev  \unspecified
...
are exported by the base library, and also by the \defining{r6rs
  arithmetic generic} library.  Unlike the procedures exported by
...
as determined by the numeric subtypes of their arguments.  Refer
to chapter~\ref{numbertypeschapter} for a detailed description of the
...
libraries that define other numerical operations.
...
consistently with the corresponding procedures whose names
...
or an expression that evaluates to a transformer FIXME, and
