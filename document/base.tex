...
where each \hyper{init} is an expression, and \hyper{body} 
is as described in section~\ref{bodiessection}. 
Any \hyper{variable} must not appear more than once in the
\hyper{variable}s.
...
than once in the set of \var{formals}.
...
where each \hyper{init} is an expression, and \hyper{body} 
is as described in section~\ref{bodiessection}.
In each \var{formals}, any variable must not appear more than once.
...
\begin{note}
  ...
  are part of the range of Unicode \textit{code points}\mainindex{code
    point}.  However, the Unicode scalar values in this range,
...
(fac 5) \ev 120
(fac 4.5) \lev \exception{\&assertion}
(fac -3) \lev \exception{\&assertion}%
\end{scheme}
\end{entry}
...
The {\cf call-with-values} procedure calls \var{producer} with no arguments and
...
expressions (see section~\ref{syntaxrulessection}), or by {\cf
  syntax-case} expressions (see library chapter~\extref{lib:syntaxcasechapter}{{\cf syntax-case}}).  It is a
