...
In most contexts, a \hyper{sequence} shall contain at least one
\hyper{expression}.  The only exceptions are the bodies of {\cf
  let-syntax} and {\cf letrec-syntax} forms that occur in a place
where a definition is allowed.
...
or an expression that evaluates to a transformer (see chapter~\ref{syntaxcasechapter}), and
...
The forms in the of a {\cf let-syntax} or {\cf letrec-syntax} are
treated, whether in definition or expression context, as if wrapped in
an implicit {\cf begin}.
Thus, internal definitions in the result of expanding the \hyper{body} have
the same region as any definition appearing in place of the {\cf
  let-syntax} form would have.
