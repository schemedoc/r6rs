...
always be converted into a equivalent {\cf letrec*}
...
The datums denoted by the \hyper{datum}s must be distinct.

\semantics
A {\cf case} expression is evaluated as follows.  \hyper{Key} is
evaluated and its result is compared in sequence against the datum denoted by
...
returned as the results of the {\cf case} expression.  If the result of
...
Each \hyper{syntax rule} must take one of the following forms:
