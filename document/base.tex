...
\hyper{transformer spec}, which must either be a {\cf syntax-rules}
...
The result(s) of the last expression in the body is(are) returned as
...
\item It may appear as a form in a \hyper{body} (see
  ...
  within the body.
\item It may appear as a regular expression and must have the shape
...
\item Both \vari{obj} and \varii{obj} are the empty list, the end of
  file object, or the unspecified value, respectively.
...
\item One of \vari{obj} and \varii{obj} is the empty list, the end of
  file object, or the unspecified value, but the other is not.
...
behavior on symbols, booleans, the empty list, the end of file object,
...
\begin{rationale} It is usually be possible to implement {\cf eq?}\ much
...
% list is the same as \schfalse{}, {\cf null?} returns \schtrue{}
...
\domain{If \var{k} is 0, \var{l} must be the empty list or a pair.
  Otherwise, \var{l} must be a chain of pairs of size at least \var{k}.}
...
\domain{The \var{list}s must all have the same length.  \var{proc}
  must be a procedure.  If the \var{list}s are non-empty,
  \var{proc} must
  take as many arguments as there are {\it list}s and must return a
  single value.}
...
A \hyper{transformer spec} in a syntax definition, {\cf let-syntax},
or {\cf letrec-syntax} can be a {\cf syntax-rules} form:
...
Each \hyper{syntax rule} must take one of the the following forms:

\begin{scheme}
(\hyper{srpattern} \hyper{template})
(\hyper{srpattern} \hyper{fender} \hyper{template})%
\end{scheme}
...
\hyper{Fender}, if present, is an expression.
The \hyper{fender} is generally useful only if the {\cf (r6rs syntax-case)}
library has been imported; see chapter~\ref{syntaxcasechapter}.
...
It is a syntax violation if the consraints of this paragraph are not met.
...
(see section~\ref{bodiessection}), at the beginning of a \hyper{library body}
(see chapter~\ref{librarychapter}), or within a 
\hyper{script body} (see chapter~\ref{scriptchapter}),
and its range is the body in which it appears, possibly but not
necessarily including code inserted into the range by macro expansion.
...
\meta{tail body} \: \arbno{\meta{declaration}} \arbno{\meta{definition}}
\>\> \meta{tail sequence}
