...
by the \rsixlibrary{base} library with level $0$, with the exception
of {\cf syntax-rules}\schindex{syntax-rules} and {\cf
  identifier-syntax}\schindex{syntax-rules}, which are exported with
level $1$.

In addition, the identifiers {\cf unquote}\schindex{unquote}, {\cf
  unquote-splicing}\schindex{unquote-splicing}, {\cf =>}\schindex{=>},
{\cf else}\schindex{else} are exported for level $0$, and the
identifiers {\cf ...}\schindex{...} and {\cf \_}\schindex{\_} are
...
However, if the implementation detects this, it must raise an
...
Binds the keyword \hyper{variable} to the value of
...
A \hyper{definition} in a \hyper{body} is known as an {\em internal definition}
\mainindex{internal definition}, as opposed to a library body
definition. 
...
expression may be immutable.  Within the same phase, a quoted literal
should evaluate to the same value (according to {\cf eqv?}) each time
it is evaluated.
\end{entry}
...
\semantics If there are no \hyper{test}s then \schtrue{} is returned.
Otherwise, the \hyper{test} expressions are evaluated from left to
right until either a \hyper{test} expression returns \schfalse, or
until the last one is reached.  Any remaining expressions are not
evaluated.  If all but the last expressions evaluate to true values,
...
\semantics If there are no \hyper{test}s then \schfalse{} is returned.
Otherwise, The \hyper{test} expressions are evaluated from left to
right until either a a \hyper{test} expression returns a true value
(see section~\ref{booleanvaluessection}), or until the last one is
reached.  Any remaining expressions are not evaluated.  If all but the
last expressions evaluate to \schfalse, the values of the last
...
Moreover, the continuation of each \hyper{init} should not be invoked
more than once.

\implresp Implementations are only required to check this restriction
to the extent that references to a \hyper{variable} during the
...
the extent that references to a \hyper{variable} during the evaluation
of the \hyper{init} expressions (using one particular evaluation order)
must be detected.  If an implementation detects a violation of the
restriction, it must raise an exception with condition type
{\cf\&assertion}.
Implementations are not required to detect that the continuation of each
...
\item \vari{Obj} and \varii{obj} are mutable objects such as pairs,
  vectors, strings, mutable records (library
  chapter~\extref{lib:recordschapter}{Records}), ports (library
  section~\extref{lib:portsiosection}{Port I/O}), hashtables (library
  chapter~\extref{lib:hashtablechapter}{Hash tables}) that denote the
  same locations in the store (section~\ref{storagemodel}).

\item \vari{Obj} and \varii{obj} are records, and 
  library section~\extref{lib:recordsequivalencesection}{Mutability
    and equivalence} specifies that {\cf eqv?}
  returns \schtrue.
...
\item \vari{Obj} is a record and \varii{obj} is not, or vice versa
  (see library chapter~\extref{lib:recordschapter}{Records}).
...
\item \vari{Obj} and \varii{obj} are mutable objects such as pairs,
  vectors, strings, mutable records (library
  chapter~\extref{lib:recordschapter}{Records}), ports (library
  section~\extref{lib:portsiosection}{Port I/O}), hash tables (library
  chapter~\extref{lib:hashtablechapter}{Hashtables}) that denote
  distinct locations.
...
bit pattern to represent both.   Moreover, they can, in certain cases,
use implementation techniques such as inlining and beta reduction to
duplicate otherwise equivalent objects.
As mentioned in section~\ref{quote},
however, a quoted literal should evaluate to the same value according
to {\cf eqv?} each time it is evaluated.
\end{rationale}
...
\domain{The \var{list}s should all have the same length.  \var{Proc}
  should be a procedure that takes as many arguments as there are
  \var{list}s and returns a single value.  \var{Proc} should not mutate
...
If multiple returns occur from {\cf map}, the return
values of earlier returns are not mutated.
...
\domain{The \var{string}s must all have the same length.  \var{Proc}
  must be a procedure.  If the \var{string}s are non-empty,
  \var{proc} must
  take as many arguments as there are {\it string}s.}
...
\domain{\var{Producer} must be a procedure that takes zero
  arguments.  \var{Consumer} should be a procedure that accepts as many
...
\domain{\var{Before} must be a procedure accepting zero arguments.
\var{Thunk} and \var{after} should also be procedures
accepting zero arguments.  These procedures may return any number of values.}
