...
and \hyper{body} is as described in section~\ref{bodiessection}.
...
\syntax
\hyper{Key} must be an expression.  Each \hyper{case clause} has one of
the following forms:
\begin{scheme}
...
denoted by the \hyper{datum}s of each \hyper{case clause} in turn, proceeding
...
constructs is identical, but they differ in the regions\index{region}
...
\item \vari{Obj} and \varii{obj} are mutable objects such as pairs,
  ...
  section~\extref{lib:portsiosection}{Port I/O}), and hashtables (library
...
For any real number \var{x} that is not an exact 0:
...
(log -1.0-0.0i)                \ev 0.0-\(\pi\)i\\\>; \textrm{if -0.0 is distinguished}%
\end{scheme}
\end{entry}
...
\domain{The \var{list}s should all have the same length.  \var{Proc}
  should be a procedure that accepts as many arguments as there are
...
vector is created.  The {\em valid indices}\index{valid indices} of a
...
\domain{\var{Producer} must be a procedure that accepts zero
