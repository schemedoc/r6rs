...
The \deflibrary{r6rs hash-tables} library operates on hash tables.
...
\begin{entry}{%
\proto{make-hash-table}{ \var{hash-function} \var{comparator}}{}
\rproto{make-hash-table}{ \var{hash-function} \var{comparator} \var{k}}}
...
Returns \schtrue{} if \var{hash-table} is a hash table, otherwise returns \schfalse.
\end{entry}
...
Applies \var{procedure} to the value in \var{hash-table} associated with \var{key}, 
or to \var{default} if \var{hash-table} does not contain an association for \var{key}.
The \var{hash-table} is then changed to associate \var{key} with the result of \var{procedure}.
...
\end{entry}
\begin{entry}{%
\proto{hash-table-clear!}{ \var{hash-table}}{}
\rproto{hash-table-clear!}{ \var{hash-table} \var{k}}}
...
Returns a list of all keys in \var{hash-table}.  The order of the list is unspecified.
Equivalent to:
\begin{scheme}
...
Returns a list of all values in \var{hash-table}.  The order of the list is unspecified.
Equivalent to:
\begin{scheme}
...
Returns the hash function used by \var{hash-table}.  For hash tables created by {\cf make-eq-hash-table} 
