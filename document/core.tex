...
(see section~\ref{bodies}), and its range is the library in which it
...
and after the declarations in a \hyper{body} (see section~\ref{bodies}).
...
\item{\tt(define \hyper{variable} \hyper{expression})}
  ...
  FIXME: \hyper{expression} hasn't been defined
...
and \hyper{body} must be according to~\ref{bodies}.
...
name.
If this restriction is violated, an exception is raised.
...
\comma\hyper{expression}, and
{\cf (unquote-splicing \hyper{expression})} may be abbreviated
\comma\atsign\hyper{expression}.
...
taking zero arguments and returning any number of values.}
...
(using {\cf call-with-current-continuation}) during the dynamic
extent.  It is only entered after all \var{before}s have been called.
...
dynamic extent.  It is exited before the \var{after}s have been called.
\end{itemize}
