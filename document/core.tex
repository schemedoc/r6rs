...
Section~\ref{coretailcontextsection} defines the rules that identify
tail calls and tail contexts in core-language programs.

\section{Core types}
\label{disjointness}

FIXME: write something on core types
...
Definitions may occur at the beginning of a \hyper{body} (that is,
...
declarations and original syntax definitions have been elided, de can
...
and \hyper{body} must be according to section~\ref{bodies}.
...
It is a syntax violation if the binding \hyper{variable} refers to is
immutable.
\end{entry}
...
\end{scheme}
and \hyper{body} should be a sequence of
one or more expressions. It is syntax violation for a \hyper{variable} to appear more
...
the numerical tower described in chapter~\ref{numberchapter}.  Unlike
...
these procedures signal an error (and should signal an error even in
unsafe mode).
...
reasonably close exact equivalent, then a violation of an
implementation restriction may be reported. FIXME
...
non-real complex arguments, they either report a violation of an
...
(/ 3)                                  \ev  1/3
(/ 0.0)                                \ev  +inf.0
(/ 1.0 0)                              \ev  +inf.0 or \var{error}
(/ -1 0.0)                             \ev  -inf.0
(/ +inf.0)                             \ev  0.0
(/ 0 0.0)                              \ev  0 or +nan.0
(/ 0.0 0)                              \ev  +nan.0 or \var{error}
(/ 0.0 0.0)                            \ev  +nan.0
\end{scheme}
...
$0.0^{\var{z}}$ is $1$ if $\var{z} = 0.0$, and $0.0$ if {\cf
  (real-part \var{z})} is positive.  Otherwise, this procedure reports
a violation of an implementation restriction or returns an unspecified
number.
...
{\cf set-car!}\ and {\cf set-cdr!} in the FIXME library (see chapter~\ref{pairmutationchapter}).
...
\domain{\var{L} must ve a plausible list up to \var{k}.  (See
  section~\ref{listargumentsection}.)}

Returns the sublist of \var{l} obtained by omitting the first \vr{k}
elements.

If \var{l} is not a plausible list, but a plausible list up to
\var{k}, no exception is raised.
\end{entry}
...
\domain{\var{L} must ve a plausible list up to and including \var{k}.  (See
  section~\ref{listargumentsection}.)}
...
If \var{l} is not a plausible list, but a plausible list up to and
including \var{k}, no exception is raised.
\end{entry}
...
Passing an immutable string to {\cf string-set!} causes an exception
...
\domain{{\cf List\coerce{}string} must be a plausible list where, for every
...
Passing an immutable vector to {\cf vector-set!} causes an exception
with condition type {\cf\&contract} to be raised.
\end{entry}
