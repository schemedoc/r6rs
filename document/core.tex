...
{\cf Case} could be defined in terms of {\cf if}, {\cf begin}, and
...
{\cf Letrec*} could be defined in terms of {\cf let}, {\cf
  unspecified}, and {\cf set!} using {\cf syntax-rules} (see
section~\ref{syntaxrulessection}) as follows:
...
The symbol {\cf <undefined>} is used in place of an expression that
returns something that when stored in a location makes it an error to
try to obtain the value stored in the location (no such expression is
defined in Scheme).
\end{entry}
...
\scheme{unquote} or \scheme{unquote-splicing} forms
...
as the outermost \scheme{quasiquote}.  The nesting level increases by one inside
