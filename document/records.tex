...
descriptor.  \defining{Protocol} must be a procedure.
\var{Protocol} is called by {\cf record-constructor} with a single
argument \var{p} and must return a procedure that creates and returns an
instance of the record type using \var{p} as described below.
...
\var{parent-constructor-descriptor} itself must be a constructor descriptor
of \var{parent-rtd}. In this case, \var{p} is a procedure that accepts
the same number
...
with {\tt v \ldots} to initialize \var{rtd}'s own fields.
...
Protocol may be \schfalse, specifying a default. This is
permissible only if either \var{rtd} is not an extension of another record
type, or, if it is, if \var{parent-constructor-descriptor} itself was
constructed with a default protocol. In the first case, \var{protocol}
...
In the second case, the default \var{protocol} returns a
...
Calls the \var{protocol} of the record-constructor descriptor
\var{constructor-descriptor} as described in
