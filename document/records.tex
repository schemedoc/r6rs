...
record-type descriptor.  An \defining{accessor} extracts from a record
...
extend another record type is called a \defining{base record type}.  A
...
target for expansion of the syntactic layers.  While the syntactic
layer can be explained and implemented in terms of the procedural
layer, the syntactic layer is used more commonly and therefore
described first.
...
the return value of {\cf (eqv? \vari{obj} \varii{obj})}, adheres to
...
The syntactic layer is provided by the \defrsixlibrary{records
  syntactic} library.  Some details of the specifications are
...
\end{scheme}
  ...
  record-type descriptor being created, in the same order. They are not used in any
  other way.
...
\begin{entry}{%
\pproto{(parent \hyper{parent name})}}
...
absence of a {\cf parent} clause implies a record type with no parent
type.  The record-type definition associated with \hyper{parent name}
must not be sealed.
  \end{entry}

\begin{entry}{%  
\pproto{(protocol \hyper{expression})}}
...
\item If there is no {\cf parent} clause and a {\cf protocol} clause,
...
\item If there is both a {\cf parent} clause and a {\cf protocol}
  ...
  constructors and the arguments to \var{n}, and this the fields of
...
\item If there is a {\cf parent} clause, but no {\cf protocol} clause,
  ...
  \hyper{constructor name} is a procedure accepting one argument for
...
below.  The role of \var{p} differs depending on the kind of record type
represented by \var{rtd}:

If \var{rtd} is a base record type, then
\var{parent-constructor-descriptor} must be \schfalse.
In this case, \var{protocol}'s argument \var{p} is a procedure
that expects one argument for every field of \var{rtd} and returns a
record with the fields of \var{rtd} initialized to these arguments.
The procedure returned by \var{protocol} 
should call \var{p} once with the number of arguments it expects and return
...
If \var{parent-constructor-descriptor} or \var{protocol} is \schfalse,
\var{protocol} must also be \schfalse, and
a default constructor descriptor is assumed as described below.

If \var{parent-constructor-descriptor} is a constructor descriptor
and \var{protocol} is a procedure, then its argument \var{p} is a
procedure that accepts the same number
of arguments as the constructor of \var{parent-constructor-descriptor}
and returns a procedure \var{new} that, when called, constructs the
record itself. The \var{new} procedure
expects one argument for every field of \var{rtd} (not including parent
fields) and returns a record with the fields of \var{rtd}
initialized to these arguments, and the fields of \var{parent-rtd} and
its parents initialized as specified by
\var{parent-constructor-descriptor}.

The procedure returned by \var{protocol} should call \var{p} once with
...
\end{scheme}
%
This passes arguments {\cf v1}, {\cf v2}, {\cf v3} to \var{p} for 
\var{parent-constructor-descriptor} and calls {\cf new}
...
\var{Protocol} may be \schfalse, specifying a default value that
accepts one argument for each field of \var{rtd} (not including the
...
(lambda (p)
  (lambda (\vari{v} \ldots \varj{v} \vari{x} \ldots \vark{x})
    (let ((new (p \vari{v} \ldots \varj{v})))
      (new \vari{x} \ldots \vark{x}))))%
\end{scheme}
%
