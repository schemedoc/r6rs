...
For two records \vari{obj} and \varii{obj}, the return value of {\cf
  (eqv? \vari{obj} \varii{obj})}, is specified as follows:

\begin{itemize}
\item If \vari{obj} and \varii{obj} have different record types (i.e.,
  their record-type descriptors are not {\cf eqv?}), {\cf eqv?}
  returns \schfalse.
\item If \vari{obj} and \varii{obj} are both mutable records of the
  same record type, and are the results of two separate calls to
  record-type constructors, then {\cf eqv?} returns \schfalse.
\item If \vari{obj} and \varii{obj} are both mutable records of the
  same record type, and are the results of a single call to a
  record-type constructor, then {\cf eqv?} returns \schtrue.
\end{itemize}
...
\begin{rationale}
  For immutable records, either \vari{obj} and \varii{obj} may have
...
below.  The role of \var{p} differs depending on the kind record type
respresented by \var{rtd}:
