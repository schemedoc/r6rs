...
These comments apply as well to {\cf eq?}, which is guaranteed to have
the same behavior on records as {\cf eqv?}.
...
\begin{note}
  Use of the {\cf parent-rtd} clause generally forces an implementation to
  delay the generation of constructor, accessor, and mutator code until
  the record-type definition is evaluated at run time, since the type of
  the parent is not generally known until then.
  The {\cf parent} clause should therefore be used instead whenever
  possible.
\end{note}
...
effects, before returning the reocrd.
\end{entry}
