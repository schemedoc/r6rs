...
reflection operations if \var{t} is opaque but \var{p} is not. An exception with
...
the same uid differs in any argument, an exception with condition type
{\cf\&contract} is raised. If
...
\schfalse{} and {\cf record-rtd} (see ``Reflection'' below) will raise
...
to obtain a mutator for a field with tag immutable will raise an
exception with condition type {\cf\&contract}.
...
a syntax defect if multiple \hyper{record clause}s of the same kind appear in a
...
The implicit-naming syntactic layer is provided by the
\deflibrary{r6rs records reflection} library.
