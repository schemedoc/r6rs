...
The \var{name} argument shall be a symbol naming the record type; it is
...
arguments in the two calls shall be {\cf eqv?}, the \var{fields}
...
The \var{fields} argument shall be a list of field specifiers. Each
field specifier shall be a list of the form {\cf (mutable \var{name})}
...
or other constructor descriptors.  \var{Rtd} shall be a record-type
descriptor.  \defining{Protocol} shall be a procedure or \schfalse.
If \schfalse, a default \var{protocol} procedure is supplied.
...
\var{parent-constructor-descriptor} shall be \schfalse.
...
\var{parent-constructor-descriptor} shall be a constructor descriptor
...
(lambda (\var{p})
  (lambda (x \ldots v \ldots)
    (let ((new (\var{p} x \ldots)))
      (\var{new} v \ldots))))
\end{scheme}

This passes some number of arguments {\tt x \ldots} to \var{p} for the
...
Calls the \var{protocol} of \var{constructor-descriptor} as described in
...
Each \hyper{record clause} shall take one of the following forms; it is
...
\item {\tt (fields \arbno{\hyper{field-spec}})}
  ...
  {\cf (immutable \hyper{field name} \hyper{accessor name})}\\
  {\cf (mutable \hyper{field name} \hyper{accessor name}\\
  \null\qquad\hyper{mutator name})}

  \hyper{Field name}, \hyper{accessor name}, and \hyper{mutator name}
  shall all be identifiers. The first form declares an immutable field
...
\item {\tt (nongenerative \hyper{uid})}
  ...
  \hyper{uid}, which shall be an \hyper{identifier}.
  If two record-type definitions specify the same \var{uid}, the
