\documentclass[twoside,twocolumn]{algol60}
%\documentclass[twoside]{algol60}

\pagestyle{headings}
\showboxdepth=0
\makeindex
\input{commands}

\def\headertitle{Revised$^{5.92}$ Scheme}
\def\integerversion{6}

% Sizes and dimensions

\texonly

\topmargin -.375in       %    Nominal distance from top of page to top of
                         %    box containing running head.
\headsep 15pt            %    Space between running head and text.

\textheight 663pt        % Height of text (including footnotes and figures, 
                         % excluding running head and foot).

\textwidth 523pt         % Width of text line.
\columnsep 15pt          % Space between columns 
\columnseprule 0pt       % Width of rule between columns.

\parskip 5pt plus 2pt minus 2pt % Extra vertical space between
                                % paragraphs.
\parindent 0pt                  % Width of paragraph indentation.
\topsep 0pt plus 2pt            % Extra vertical space, in addition to 
                                % \parskip, added above and below list and
                                % paragraphing environments.

\oddsidemargin  -.5in    % Left margin on odd-numbered pages.
\evensidemargin -.5in    % Left margin on even-numbered pages.

\endtexonly

%% End of sizes and dimensions

\begin{document}

\parindent 0pt %!! 15pt                    % Width of paragraph indentation.

\input{first}   \par
\input{intro}   \par
\vskip 2ex
\clearchaptergroupstar{Description of the language} %\unskip\vskip -2ex
\input{struct}  \par
\input{numbers} \par
\input{lex}	\par
\input{basic}	\par
\input{terminology} \par
\input{library} \par
\input{scripts} \par
\input{expansion} \par
\input{base}	\par
\clearchaptergroupstar{Description of the standard libraries}
\input{unicode} \par
\input{bytes}   \par
\input{list}    \par
\input{records} \par
\input{exc}     \par
\input{io}      \par
\input{arith}   \par
\input{syntax-case} \par
\input{hashtable} \par
\input{enum}    \par
\input{misclib} \par
\input{complib} \par
\input{eval}    \par
\input{setcar}  \par
\input{r5rscompat} \par
\clearchaptergroupstar{Appendices}
\appendix
\input{sem}	\par
\input{derived} \par
\input{repository} \par
\input{example}	\par
%\newpage                   %  Put bib on it's own page (it's just one)
%\twocolumn[\vspace{-.18in}]%  Last bib item was on a page by itself.
\renewcommand{\bibname}{References}
\input{bib}	\par

\vfill\eject


\texonly
\newcommand{\indexheading}{Alphabetic index of definitions of
  concepts, keywords, and procedures}
\newcommand{\indexintro}{The principal entry for each term, procedure,
  or keyword is listed first, separated from the other entries by a
  semicolon.}

\printindex
\endtexonly

\htmlonly
\extrapart{Alphabetic index of definitions of concepts, keywords, and procedures}
\inputindex
\endhtmlonly

\end{document}
