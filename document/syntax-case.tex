...
provided by {\cf syntax-rules} (section~\ref{syntax-rules}).

Because {\cf syntax-case} does not require literals, including quoted
lists or vectors, to be copied or even traversed, it preserves sharing
...
\defining{variable transformer}.
In a macro use that consists only of a keyword, \var{proc} is called
with a wrapped syntax object representing just the keyword
reference, and its return value replaces just the reference.

If a keyword associated with a variable transformer appears on the
left-hand side of a {\cf set!} expression, \var{proc} is called with a
...
\hyper{Fender} and \hyper{output expression} shall be
\hyper{expression}s.
...
definition for {\cf letrec} in appendix~\ref{derivedformsappendix}.
\end{entry}
...
In this case, uses of the identifier by itself will be replaced by
\hyperi{template}, and uses of {\cf set!} with the identifier will be
