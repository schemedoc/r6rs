...
\library{r6rs ports} library.\schindex{eof-object}\schindex{eof-object?}
\end{entry}
...
\domain{\var{Thunk} must be a procedure that takes no arguments.}  The
...
\implresp The implementation must check the restrictions on \var{thunk}
to the extent performed by applying it as described.
\end{entry}
...
\var{textual-output-port}. The unspecified value is
returned.
