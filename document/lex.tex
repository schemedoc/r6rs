...
``{\tt\#x1c}'' are syntactic data representing the exact integer 28,
and the syntactic data ``{\tt(8 13)}'', ``{\tt( 08 13 )}'', ``{\tt(8 .\
  (13 .\ ()))}''
all represent a list containing the integers 8 and 13. 
Syntactic data that denote equal objects (in the sense of {\cf
...
syntax are syntactic data in the read syntax, and thus represent data.
Besides the formal account of the syntax, this section also describes
what datum values are denoted by these syntactic data.
...
Case is significant except in boolean data, number data, and
...
(cf.\ Section~\ref{datayntax}) with {\tt \#;}\sharpindex{;}, possibly with
...
\begin{rationale}
  ...
  cannot be used to comment out an arbitrary datum or set of data;
  it works only when none of the data include a string with an
...
List and pair data, denoting pairs and lists of values
...
Vector data, denoting vectors of values (see
...
Bytevector data, denoting bytevectors (see
