...
\subsection{Quote}\unsection
\label{quotesection}

\begin{entry}{%
\pproto{\singlequote\hyper{datum}}{\exprtype}}

\mainschindex{'} singlequote\hyper{datum} is an syntactic abbreviation
for {\cf (quote \hyper{datum})}.  The two notations are equivalent in
all respects.

As noted in section~\ref{storagemodel}, it is an error to alter a
constant using a mutation procedure like {\cf set-car!}\ or {\cf
  string-set!}.
\end{entry}
