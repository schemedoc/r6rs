...
\defining{datum}) may also correspond to other S-expressions.  For example, the
...
Another way to indicate a comment is to prefix a \hyper{S-expression}
(cf.\ Section~\ref{datumsyntax}) with {\tt \#;}, possibly with
whitespace before the \hyper{S-expression}.  The comment consists of
the comment prefix {\tt \#;} and the \hyper{S-expression} together.  (This
notation is useful for ``commenting out'' sections of code.)

Bqlock comments may be indicated with properly nested {\tt
  \#|} and {\tt |\#} pairs.
...
base-library expression evaluating to a vector.
...
base-library expression evaluating to a bytes object.
