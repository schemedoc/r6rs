...
\domain{For {\cf bytevector-uint-set!}, \var{n} must be an exact
  integer object in the interval $\{0, \ldots, 256^{\var{size}}-1\}$.}
...
\domain{\var{Size} must be a positive exact integer object.  For {\cf
  ...
  integer objects in the interval $\{0, \ldots, 256^{\var{size}}-1\}$.  For
  {\cf sint-list->bytevector}, \var{list} must be a list of exact
  integer objects in the interval $\{-256^{\var{size}}/2, \ldots,
...
\domain{\var{K} must be a valid index of \var{bytevector}; so must
  $\var{k} + 1$. For {\cf bytevector-u16-set!} and {\cf
    bytevector-u16-native-set!}, \var{n} must be an exact integer object in
...
\domain{$\{\var{k}, \ldots, \var{k}+ 3\}$ must be valid indices of
  ...
  For {\cf bytevector-u32-set!} and {\cf
    bytevector-u32-native-set!}, \var{n} must be an exact integer
  object in
...
\domain{$\{\var{k}, \ldots, \var{k}+ 7\}$ must be valid indices of
  ...
  For {\cf bytevector-u64-set!} and {\cf
    bytevector-u64-native-set!}, \var{n} must be an exact integer
  object in
...
\rproto{utf16->string}{ bytevector endianness endianness-mandatory?}{procedure}}

\domain{\var{Endianness} is specified must be the symbol {\cf big} or
...
\rproto{utf32->string}{ bytevector endianness endianness-mandatory?}{procedure}}
