...
various sizes.  Therefore, the \deflibrary{r6rs bytevector} library
...
\begin{note}
  Implementators are encouraged to use widely accepted designations
...
This may be any \hyper{endianness symbol}, specifically a symbol other
...
\begin{entry}{%
\proto{bytevector-copy!}{ source source-start target target-start n}{procedure}}
...
\domain{\var{K} must be a valid index of \var{bytevector}; so must $\var{k}
  + 1$.}
...
\domain{$\{\var{k}, \ldots, \var{k}+ 3\}$ must be valid indices of \var{bytevector}.}.
...
\domain{$\{\var{k}, \ldots, \var{k}+ 7\}$ must be valid indices of \var{bytevector}.}
...
\proto{bytevector-ieee-single-set!}{ bytevector k x endianness}{procedure}}
...
elements \var{k} through $\var{k}+3$ of \var{bytevector}, and returns
\unspecifiedreturn.
\end{entry}

\begin{entry}{%
\proto{bytevector-ieee-double-native-set!}{ bytevector k x}{procedure}
\proto{bytevector-ieee-double-set!}{ bytevector k x endianness}{procedure}}
...
elements \var{k} through $\var{k}+7$ of \var{bytevector}, and returns
\unspecifiedreturn.
\end{entry}
