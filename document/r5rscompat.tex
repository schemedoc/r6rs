...
return integers.  If \vari{n}/\varii{n} is an integer:
\begin{scheme}
    (quotient \vari{n} \varii{n})   \ev \vari{n}/\varii{n}
    (remainder \vari{n} \varii{n})  \ev 0
    (modulo \vari{n} \varii{n})     \ev 0
\end{scheme}
If \vari{n}/\varii{n} is not an integer:
\begin{scheme}
    (quotient \vari{n} \varii{n})   \ev \var{n_q}
    (remainder \vari{n} \varii{n})  \ev \var{n_r}
    (modulo \vari{n} \varii{n})     \ev \var{n_m}
\end{scheme}
where \var{n_q} is $\vari{n}/\varii{n}$ rounded towards zero,
$0 < |\var{n_r}| < |\varii{n}|$, $0 < |\var{n_m}| < |\varii{n}|$,
\var{n_r} and \var{n_m} differ from \vari{n} by a multiple of \varii{n},
\var{n_r} has the same sign as \vari{n}, and
\var{n_m} has the same sign as \varii{n}.

Consequently, for integers \vari{n} and \varii{n} with
