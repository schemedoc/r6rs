...
first alternative is a program with a store and an expression. The second alternative is an exception, and the third is
...
The \nt{f} describes the formals for \sy{lambda} expressions. (The \sy{dot} is used instead of a period for procedures that accept an arbitrary number of arguments, in order to avoid meta-circular confusion in our PLT Redex model.) 
...
they are evaluated first, and turn the datums into calls to either \va{cons} or \va{consi}, via the metafunctions $\mathscr{Q}_i$ and $\mathscr{Q}_m$.
...
$v$ can occur in the test to an \sy{if} expression.
...
test. Once that is a value, the rules for \sy{if} reduce
an \sy{if} expression to its consequent if the test is not
\semfalse{}, and to its alternative if it is.
