...
This section describes the \defrsixlibrary{arithmetic fx} library,
...
1.}  The {\cf fxcopy-bit} procedure returns the result of the following computation:
\begin{scheme}
...
This section describes the \defrsixlibrary{arithmetic flonum} library.
...
Returns \vari{fl} raised to the power \varii{fl}.  Either \vari{fl}
should be non-negative, or, if \vari{fl} is negative, \varii{fl}
should be an integer object; otherwise, the result may be a NaN, or may
be some meaningless flonum.  If \vari{fl} is zero, then the result is
zero.
\end{entry}
...
Returns the exact integer object that is the result of the following
computation:
\begin{scheme}
(do ((result 0 (+ result 1))
