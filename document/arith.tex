...
\schtrue{} if the \varii{fx}th bit is set in the two's complement
...
number represented by the bits at the positions from \varii{fx} to
...
the result of replacing in \variv{fx} the bits at positions from
\varii{fx} to $\variii{fx}$ by the corresponding bits in \vari{fx}, which
...
bits at positions from \varii{fx} to \variii{fx} by \variv{fx} bits
...
Returns \vari{fl} raised to the power \varii{fl}.  If \vari{fl} is
...
Some procedures allow extracting \defining{bit fields}, i.e., number objects
...
\schtrue{} if the \varii{ei}th bit is set in the two's complement
...
number represented by the bits at the positions from \varii{ei} to
...
the result of replacing in \variv{ei} the bits at positions from
\varii{ei} to $\variii{ei}$ by the corresponding bits in \vari{ei}, which
is the fixnum result of the following computation:
%
\begin{scheme}
(let* ((to    \vari{ei})
...
bits at positions from \varii{ei} to \variii{ei} by \variv{ei} bits
towards the more significant bits, which is the result of the
following computation:
%
\begin{scheme}
...
the result obtained from \vari{ei} by reversing the bit field
