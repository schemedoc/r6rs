...
NaNs, an implementation must report a violation of an implementation
restriction whenever it is unable to represent an infinity or NaN as
required by the specification.  This requirement also applies to
conversions between numbers and external representations, including
the reading of program source code.

FIXME: specify how the violation is reported
...
Both of these procedures return the unique fixnum that is congruent
...
If the argument \var{fx} is non-negative, these procedures return the
...
These procedures return the index of the least significant 1 bit in
