...
the car and cdr fields of pairs.  In programs that use this library,
...
<<<<<<< .mine
may not always be the head of a list.  Any procedures that are defined
in this report and are specified as requiring a list argument
must check whether that argument appears to be a list.
This checking is complicated by
the fact that many procedures that accept lists, such as {\cf map} or
...
Informally, a plausible list is an object that appears as
...
\domain{\var{Proc} must be a procedure taking as many arguments
