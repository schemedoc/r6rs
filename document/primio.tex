...
The Primitive I/O layer handles only blocking-I/O.
...
\item {\cf (\var{get-position})}
  ...
  When present, returns the current position in the 
...
\item {\cf (\var{set-position!} \var{pos})}
  ...
  When present, moves to position \var{pos} (which
...
efficient size of the read operations on this reader. This result is typically the
block size of the buffers of the operating system. As such, it is only a
...
For a reader created by {\cf make-simple-reader}, this procedure
...
Returns \schtrue{} if reader supports the reader-get-position
...
For a reader created by {\cf make-simple-reader}, this procedure tail-calls the
...
For a reader created by {\cf mmake-simple-reader}, this procedure tail-calls the
...
Returns a \defining{bytes reader} that uses a copy of
...
The \var{file-options} object, may determine various aspects of the returned reader
...
\item {\cf (\var{set-position!} \var{pos})}
  ...
  When present, moves to position \var{pos} (which must be a
...
\proto{writer-descriptor}{ writer}{procedure}}
  ...
  For a writer created by {\cf make-simple-writer}, returns the value of the
...
For a writer created by {\cf make-simple-writer}, this procedure tail-calls the
...
\var{writer}, returns the current position in the byte stream as
...
\var{writer}, moves to position \var{pos} (which must be a non-negative
exact integer) in the stream.
   
For a writer created by {\cf make-simple-writer}, this procedure calls the
...
The \var{writer} argument must be a bytes writer. 
