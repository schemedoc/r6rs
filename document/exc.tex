...
that is either a simple condition or a compound condition is called
simply \defining{condition}.  Compound conditions form a type disjoint
...
\domain{\var{Cond} must be a condition.}  The {\cf simple-condition}
...
returned list may be immutable.  If the returned list is modified, the
...
\item \hyper{Constructor} is bound to a default constructor for the
  ...
  (including parent types) and returns a condition object initialized
  to those arguments, with the field values for the parent coming
  before those of the extension in the argument list.

\item \hyper{Predicate} is bound to a predicate that identifies
  conditions of type \hyper{condition-type}, or with any of its
  subtypes.
