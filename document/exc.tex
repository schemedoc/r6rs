...
The section describes's Scheme \deflibrary{r6rs
...
Returns a new condition type. \var{Id} shall
be a symbol that serves as a symbolic name for the condition type.
\var{Parent} shall itself be a condition type. \var{Field-names} shall
be a list of symbols. It identifies the fields of the conditions
associated with the condition type.

\var{Field-names} shall be disjoint from the field names of
...
type \var{type}. \var{Field-name} shall be a field name. There shall be
...
\var{Field-name} shall be a symbol.
Moreover, \var{condition} shall belong to a condition type which has a
field name called \var{field-name}, or one of its (direct or indirect)
supertypes shall have the field.  The {\cf condition-ref} procedure returns the value
...
\var{Condition} shall be a condition belonging to \var{condition-type}.
...
\hyper{supertypes}, and \hyper{predicate} shall all be identifiers.
Each \hyper{field-spec} shall be of the form
%
\begin{scheme}
(\hyper{field} \hyper{accessor})%
\end{scheme}
%
where both \hyper{field} and \hyper{accessor} shall be identifiers.

\semantics
The {\cf define-condition-type} form defines an identifier
\hyper{condition-type} to some value describing a new condition type.
\hyper{Supertype} shall be the name of a previously defined condition
type.

The {\cf define-condition-type} form also defines \hyper{predicate} to a
predicate that identifies conditions associated with that type, or
with any of its subtypes.

The {\cf define-condition-type} form defines each \hyper{accessor} to a
...
Returns a condition value. Each \hyper{type-field-binding} shall be of
the form
%
\begin{scheme}
...
x
The \hyper{expression} are evaluated in some unspecified order; their values
...
Each \hyper{type-field-binding} shall contain field bindings for all
