...
section~\extref{report:dynamicenvironmentsection}{Dynamic environment}.
...
\begin{entry}{%
\pproto{(guard (\hyper{variable} \hyperi{cond clause} \hyperii{cond clause} \dotsfoo)  \hyper{body})}{\exprtype}}
\mainschindex{guard}
...
\hyper{cond clause}'s \hyper{test} evaluates to \schfalse{} and there is no else clause, then
...
\domain{The \var{condition}s must be conditions.}  The {\cf condition}
...
\domain{\var{condition} must be a condition.}  The {\cf simple-conditions}
...
\domain{\var{Rtd} must be a record-type descriptor of a subtype of
  {\cf\&condition}.  \var{Proc} must be a procedure and should accept
