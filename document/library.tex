...
Note that when base-library {\cf begin} forms occur in a body, they are spliced into the
...
Scheme programs can define and use new derived expressions or
...
Derived expressions and definitions typically have the form:
\begin{scheme}
(\hyper{keyword} \hyper{datum} \dotsfoo)%
\end{scheme}%
where \hyper{keyword} is an identifier that uniquely determines the
type of expression or definition.  This identifier is called the {\em syntactic
...
(library (my-helpers values-stuff)
  ...
  (import r6rs (import (for (my-helpers id-stuff) expand)))
