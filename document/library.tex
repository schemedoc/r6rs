...
A \hyper{library name} have the following form:
...
\begin{scheme}
\hyper{empty}
(\hyper{subversion~reference} \ldots)%
\end{scheme}
...
(>= \hyper{subversion})
(<= \hyper{subversion})
(and \hyper{subversion~condition} \ldots)
(or \hyper{subversion~condition} \ldots)
...
\hyper{import~set}, an exception is raised.
\item The {\cf prefix} adds the \hyper{identifier} prefix to each
...
library. For example, if a library provides functions that are called
...
\mainindex{phase}
%
A \emph{phase} is a time relative to a particular library that
expressions are evaluated. Within a library, top-level expressions and
the right-hand sides of {\cf define} forms are evaluated at run time,
that is, at phase $0$ for to the enclosing library. The right-hand
sides of {\cf define-syntax} forms are evaluated at expand time, which
is phase $1$ for the enclosing library. When a {\cf let-syntax}
expression appears as a phase $n$ expression, the right-hand sides of
its bindings are evaluated as phase $n+1$ expressions.

An \emph{instance} of a library corresponds to an evaluation of its
definitions and expressions in a particular phase relative to another
library. For example, if a top-level expression in a library refers to
an export from another library, then it refers to the export from an
instance of the latter library at phase $0$ (relative to the former
library). But if evaluating the right-hand side of a {\cf
  define-syntax} expression refers to the same export of the other
library, then it it refers to the export from an instance of the
latter library at phase $1$ (relative to the former library).
...
... FIXME ... from the {\cf (r6rs base)} library, which are exported
with level $1$, and all bindings from the composite {\cf (r6rs)}
library (see library chapter~\ref{lib:complibchapter}), which are
...
library in all phases above $0$. An implementation is allowed to
...
time, or it may allow the reference. Thus, a portable library must
reference identifiers only in phases consistent with the declared
levels, and the library's meaning must not depend on whether the
instances of a library are distinguished or shared across phases or
{\cf library} expansions.
...
In particular, if a program and its libraries avoids the {\cf (r6rs)}
and {\cf (syntax-rules)} libraries, and if the program and libraries
