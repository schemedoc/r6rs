...
importing libraries.
All other variables defined within a library are mutable within the library,
but implicitly exported variables are immutable in the importing libraries.
That is, any change after the initial assignment to the value of an
implicitly exported variable is not reflected by the references inserted
by an exported macro outside of the exporting library, and 
an exception with condition type {\cf\&syntax} is raised if an implicitly
exported variable appears on the left-hand side of a {\cf set!} expression
in any code produced by an exported macro outside of the library in which
the variable is defined.

\begin{rationale} 
Determining the set of implicit exports is generally impossible without
expanding all uses of an exported macro in all importing libraries, i.e.,
not until after the exporting library has been expanded, possibly
during a separate compilation step.
Allowing implicit exports to be mutable everywhere would therefore be generally
equivalent to making all library bindings mutable, if any macros are
exported, and making all implicit exports immutable even within the
exporting library would be generally equivalent to making all library
bindings immutable.
Making such bindings mutable inside but immutable outside of the library
is a compromise between these two extremes that permits both
programmers and compilers to locate the points at which all variables are
assigned while allowing a programmer to make use of mutable local variables.
\end{rationale}
...
control over imported bindings is not necessary or desireable. The {\cf r6rs} module
...
Scheme programs can define and use new derived expressions,
...
\item A set of forms ({\cf define-syntax},
  section~\ref{define-syntax}, {\cf let-syntax} and {\cf
    letrec-syntax}; see section~\ref{let-syntax}) used to create bindings
