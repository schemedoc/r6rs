...
\end{scheme}
%
The second form is synonymous with the first.  The syntax of an
\hyper{import spec} is interpreted outermost to innermost
...
(only \hyper{import~set} \hyperi{identifier} \hyperii{identifier} \ldots)
(except \hyper{import~set} \hyperi{identifier} \hyperii{identifier} \ldots)
(prefix \hyper{import~set} \hyper{identifier})
(rename \hyper{import~set} (\hyperi{identifier} \hyperii{identifier})
                    (\hyperiii{identifier} \hyperiv{identifier}) \ldots)%
\end{scheme}
...
In cases where an \hyper{import~set} starting with {\cf only}, {\cf
  except}, {\cf prefix}, or {\cf rename}, or a
\hyper{version~reference} starting with {\cf and}, {\cf or}, or {\cf
  not} occurs in the place of a \hyper{library~reference}, and fits
the form of a \hyper{library~reference}, the interpretation as an
\hyper{import~set} or a \hyper{version~reference} takes precedence.  A
\hyper{import~spec} using {\cf library} can be used to resolve the
ambiguity.
...
(and \hyperi{version reference} \hyperii{version reference} \ldots)
(or \hyperi{version reference} \hyperii{version reference} \ldots)
...
(>= \hyper{sub-version})
(<= \hyper{sub-version})
(and \hyperi{sub-version~reference} \hyperii{sub-version~reference} \ldots)
(or \hyperi{sub-version~reference} \hyperii{sub-version~reference} \ldots)
(not \hyperi{sub-version~reference})%
\end{scheme}
