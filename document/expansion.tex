...
residualizes the equivalent of a {\cf letrec*} form from the defined variables,
expanded right-hand-side expressions, and expanded body expressions.
For each body expression \hyper{expr} that appears before a variable definition
...
left-hand side and the equivalent of {\cf (begin \hyper{expr} (unspecified))}
on the right-hand side, so that
left-to-right evaluation order is preserved.
The {\cf begin} wrapper allows \hyper{expr} to evaluate to zero or more
values.
