...
\rn{6}.
Despite this, the {\tt ceiling}, {\tt floor}, {\tt round}, and
{\tt truncate} procedures (and their {\tt fl}-prefixed counterparts) return an
...
We observe that the integer values of the mathematical equivalents of
these procedures are, in fact, infinite for infinite inputs, and
that while infinities are not considered integers, they might represent
infinite integers, so the extension to infinities, at least, makes sense.
...
representation.  Moreover, symbols could be distinguish internally by
...
now describes three comment forms: In addition to {\cf;}, {\tt \#|}
...
They are useful for commenting out alternative versions of a form
(say one that is more clear but less efficient) or to include
forms that may be required, say, only if some future extension is
made.
...
Some Scheme implementations feature module systems that allow importing a
module's bindings into a local environment. 
While local imports can be used to limit the scope of an import and thus
leads to more modular code (and less need for the prefixing and renaming
of imports), the existence of local imports would mean that a library's
dependencies are not readily apparent from a library header, which some of
the editors feel is important.
...
are fixed---they cannot be the result of macro expansion.  As with
the decision to omit local import, this reflects the desire of some
editors to make library dependencies readily apparent from the library
header.
