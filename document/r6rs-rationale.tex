...
Note that, if a program uses only {\cf syntax-rules} or
{\cf identifier-syntax} forms as the
right-hand sides of keyword definitions and keyword binding forms, does
does not use {\cf syntax-rules} or {\cf identifier-syntax} in any
other context, and avoids the use of {\cf for} in all {\cf import}
forms, then the program does not depend on whether instances are
...
which references are allowed at any phase regardless of the phase
declarations.
