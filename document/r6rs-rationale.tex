...
appendix~\extref{app:caseinsensitivityappendix}{Optional case
  insensitivity} allow specifying that a section of the code (or other
...
The rationale for {\cf\sharpsign;} is not as readily apparent: It is
guaranteed syntactically to comment out a single datum---no more, and
no less. Arguably, {\cf\sharpsign;} is most useful during development
and debugging; even so, programmers sometimes develop and debug a
single piece of code concurrently on multiple systems, in which case a
standard notation for commenting out a datum is useful.
...
not preclude it from being added later, the editors decided to omit
...
Note that, if a program and its libraries avoid the \thersixlibrary{}
and \rsixlibrary{syntax-case} libraries, and if the program and libraries
never use the {\cf for} import form, and if all macros are defined using
{\cf syntax-rules}, then the program does not depend
...
appear.  Composition of definitions with expressions therefore must be
...
to mark conditionals with side effects, to avoid one-armed {\cf if}
expressions, while they use two-armed {\cf if} expressions primarily for
...
types.  The syntactic layer is designed to allow expansion-time
determination of record characteristics (at least in the
...
to represent that ADT by a record type, and allows one of the classes
