...
\domain{\var{Proc} must be a procedure taking a single argument.}  {\cf Find} applies
...
tail call of \var{proc} on the last elements.  If \var{proc}
returns \schfalse{} on any set of elements, {\cf forall} returns
\schfalse{} after the first such application of \var{proc}.  If
...
that value after the first such application of \var{proc}.  If
...
some time after $t_n$ and before the procedure returns.  (See
...
\domain{\var{Proc} must be a procedure taking a single argument.}  {\cf Filter} successively applies
...
finds one for which \var{proc} returns a true value.  {\cf
