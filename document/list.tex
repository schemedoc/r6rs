...
This section describes the \deflibrary{r6rs lists} library.

\begin{entry}{%
\proto{find}{ proc list}{procedure}}

\domain{\var{Proc} must be a procedure; it must take a single argument
  if \var{list} is non-empty.}  {\cf Find} applies
...
\domain{The \var{l}s must all be the empty list, or chains of pairs of
  sizes according to the condition specified below.
  \var{Proc} must be a procedure; it must take a single argument if
  the \var{l}s are non-empty.}

{\cf Forall} applies \var{proc} element-wise to the
...
\var{l}s must all be lists.  If \var{proc}
...
{\cf Exists} applies \var{proc} element-wise to the
...
{\cf Fold-left} iterates the \var{kons} procedure over an
...
\domain{ If
  ...
  procedure; if the \var{list}s are non-empty, it must take one more
  argument than there are {\it list}s.}
{\cf Fold-right} iterates the \var{kons} procedure over
...
(not the empty list) is returned; in that case, \var{al} must be a
list.  {\cf Assp} successively applies
