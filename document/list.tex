...
This chapter describes the \defrsixlibrary{lists} library.

\begin{entry}{%
\proto{find}{ proc list}{procedure}}

\domain{\var{Proc} should be a procedure that takes a single argument
  and returns a single value.  \var{Proc} should not mutate \var{list}.}
...
\domain{The \var{list}s should all have the same length, and
  \var{proc} should be a procedure that accepts $n$ arguments and
...
\domain{\var{Proc} should be a procedure that accepts a single argument
  and returns a single value. \var{Proc} should not mutate \var{list}.}
...
\domain{The \var{list}s should all have the same
  length.  \var{Combine} should be a
...
\var{list} further.  \var{Proc} is always called in the same dynamic environment 
...
\domain{\var{Alist} (for ``association list'') should be a list of pairs.
  \var{Proc} should be a procedure that takes a single argument
  and returns a single value.  \var{Proc} should not mutate \var{alist}.}
