...
and compares those keys with eq? If an argument is given, then the initial 
...
Returns a newly allocated hash table that accepts arbitrary objects as keys, 
and compares those keys with eqv? If an argument is given, then the initial 
capacity of the hash table is set to \var{k} elements.
...
\begin{entry}{%
\proto{hash-table-ref}{ \var{hash-table} \var{key}}{}
\proto{hash-table-ref}{ \var{hash-table} \var{key} \var{default}}}

Returns the value in \var{hash-table} associated with \var{key}.  If \var{hash-table} does not contain an association for \var{key}, then either an exception [TODO] is raised, or if the \var{default} argument is provided, its value is returned.
...
Equivalent to: \begin{scheme}(hash-table-ref hash-table key \schfalse)\end{scheme}
\end{entry}
...
\begin{entry}{%
\proto{hash-table-update!}{ \var{hash-table} \var{procedure}}{}
...
If \var{hash-table} does not contain an association for \var{key}, then either an exception [TODO] is raised, or if the \var{default} argument is provided, its value is passed to \var{procedure}.
\end{entry}
...
Some procedures could perhaps be omitted, e.g. there are four retrieval procedures: -ref, -ref/call, -ref/thunk, -get.  Which one(s) should be omitted, if any?  Procedures which take failure thunks are candidates for omission.
